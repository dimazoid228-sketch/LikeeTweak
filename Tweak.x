...
- (void)updateAdCards
{
    ...
    /*
     * Эти классы являются UI-классами рекламных карточек,
     * которые мы уже нашли во Flex.
     */
    Class cardClass =
        NSClassFromString(
    ...
    /*
     * BGServerAdImageView намеренно НЕ скрываем.
     *
     * Он слишком общий и мог затрагивать обычный UI,
     * из-за чего у тебя пропадали комментарии.
     */
           ...
           adClass:(Class)adClass
{
    ...
    if ([view isKindOfClass:adClass]) {
        /*
         * Не скрываем view размером почти со весь экран.
         * Такой объект скорее всего является контейнером
         * ленты/экрана, а не самой рекламной карточкой.
         */
        CGFloat screenWidth =
        ...
        CGFloat viewArea =
            fabs(view.bounds.size.width) *
            fabs(view.bounds.size.height);
        if (screenArea > 0.0 &&
            viewArea >= screenArea * 0.85) {
            NSLog(
                @"[LikeeTweak] SKIP LARGE AD VIEW %@ %.0fx%.0f",
             ...
             * Не делаем return.
             *
             * Продолжаем искать рекламные элементы
            ...
            /*
             * Если это сама рекламная карточка,
             * дальше внутрь уже идти не нужно.
             */
            return;
        }
    }
    /*
     * Копируем массив, чтобы не получить проблемы,
     * если Likee изменит иерархию во время обхода.
     */
    NSArray *subviews =
        [view.subviews copy];
